...
class BaseModel(abc.ABC):
    '''
    SingaAuto's base model class that SingaAuto models must extend.

    SingaAuto models must implement all abstract methods below, according to the specification of its associated task (see :ref:`tasks`).
    They configure how this model template will be trained, evaluated, tuned, serialized and served on SingaAuto.
    ...
    These knob values are chosen by SingaAuto based on the model's knob configuration (defined in :meth:`singaauto.model.BaseModel.get_knob_config`).
    ...
    :type knobs: :obj:`singaauto.model.Knobs`
    '''
    ...
    def train(self, dataset_path: str, shared_params: Optional[Params] = None, **train_args):
        ...
        :param shared_params: Dictionary mapping parameter names to values, as produced by your model's :meth:`singaauto.model.BaseModel.dump_parameters`.
        '''
        raise NotImplementedError()
    ...
    def dump_parameters(self) -> Params:
        ...
        This dictionary must conform to the format :obj:`singaauto.model.Params`.
        This will be used to save the trained model in SingaAuto.
    ...
    def load_parameters(self, params: Params):
        '''
        Loads this model instance with previously trained model parameters produced by your model's :meth:`singaauto.model.BaseModel.dump_parameters`.
