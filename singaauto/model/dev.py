...
from singaauto.constants import ModelDependency, Budget
from singaauto.advisor import ParamsType, Proposal, TrialResult, make_advisor
from singaauto.predictor import get_ensemble_method, Query, Prediction
from singaauto.param_store import FileParamStore, ParamStore
from singaauto.redis import ParamCache, TrainCache, InferenceCache
...
def test_model_class(model_file_path: str, model_class: str, task: str, dependencies: Dict[str, str], 
    ...
    on a given dataset. The model's methods will be called in an manner similar to that in SingaAuto.
...
def _check_model_class(py_model_class):
    if not issubclass(py_model_class, BaseModel):
        raise Exception('Model should extend `singaauto.model.BaseModel`')
