...
from singaauto.constants import ModelAccessRight, ModelDependencies, Budget, BudgetOption, \
                            InferenceBudget, InferenceBudgetOption, UserType
from singaauto.model import Params, BaseModel

class SingaAutoConnectionError(ConnectionError): pass

DOCS_URL = 'https://nginyc.github.io/singaauto/docs/latest/docs/src/python/singaauto.client.Client.html'
...
class Client():
    ...
    :param admin_host: Host of SingaAuto Admin
    :param admin_port: Port of SingaAuto Admin
    '''
    def __init__(self, admin_host: str = os.environ.get('SINGAAUTO_ADDR', 'localhost'),
    ...
    def ban_user(self, email: str) -> Dict[str, Any]:
        '''
        Bans a SingaAuto user, disallowing logins.
    ...
    def create_dataset(self, name: str, task: str, dataset_path: str = None, dataset_url: str = None) -> Dict[str, Any]:
        '''
        Creates a dataset on SingaAuto, either by uploading the dataset file from your filesystem or specifying a URL where the dataset file can be downloaded.
    ...
    def create_model(self, name: str, task: str, model_file_path: str, model_class: str, dependencies: ModelDependencies = None, 
        ...
        :param model_class: The name of the model class inside the Python file. This class should implement :class:`singaauto.model.BaseModel`
        :param dependencies: List of Python dependencies & their versions
        :param access_right: Model access right
        :param docker_image: A custom Docker image that extends ``singaautoai/singaauto_worker``, publicly available on Docker Hub.
        :returns: Created model as dictionary

        Refer to :ref:`model-development` for more details on how to write & test models for SingaAuto.
        ...
        If a model's ``access_right`` is set to ``PUBLIC``, this model will be publicly available to all other users on SingaAuto for training
    ...
    def download_model_file(self, model_id: str, out_model_file_path: str) -> Dict[str, any]:
        '''
        Downloads the Python model class file for the SingaAuto model.
    ...
    def get_available_models(self, task: str = None) -> List[Dict[str, Any]]:
        '''
        Lists all SingaAuto models available to the current user, optionally filtering by task.
    ...
    def create_train_job(self, app: str, task: str, train_dataset_id: str, val_dataset_id: str, 
        ...
        '''
        Creates and starts a train job on SingaAuto. 
        ...
        :param train_dataset_id: ID of the train dataset, previously created on SingaAuto
        :param val_dataset_id: ID of the validation dataset, previously created on SingaAuto
        ...
        ``<budget_type>`` is one of :class:`singaauto.constants.BudgetOption` and 
    ...
    def get_train_jobs_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        '''
        Lists all of user's train jobs on SingaAuto.
    ...
    def get_train_jobs_of_app(self, app: str) -> List[Dict[str, Any]]:
        '''
        Lists all of current user's train jobs associated to the app name on SingaAuto.
    ...
    def create_inference_job(self, app: str, app_version: int = -1, budget: InferenceBudget = None) -> Dict[str, Any]:
        '''
        Creates and starts a inference job on SingaAuto with the best-scoring trials of the associated train job. 
        ...
        ``budget`` should be a dictionary of ``{ <budget_type>: <budget_amount> }``, where 
        ``<budget_type>`` is one of :class:`singaauto.constants.InferenceBudgetOption` and 
        ``<budget_amount>`` specifies the amount for the associated budget option.
    ...
    def get_inference_jobs_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        '''
        Lists all of user's inference jobs on SingaAuto.
    ...
    def get_inference_jobs_of_app(self, app: str) -> List[Dict[str, Any]]:
        '''
        Lists all inference jobs associated to an app on SingaAuto.
    ...
    def stop_all_jobs(self):
        '''
        Stops all train and inference jobs on SingaAuto. 
