...
if __name__ == '__main__':
    ...
    env.append({'name': 'KAFKA_MESSAGE_MAX_BYTES', 'value': '10000000'})
    env.append({'name': 'KAFKA_FETCH_MAX_BYTES', 'value': '10000000'})
    container.setdefault('env', env)
    template.setdefault('spec', {'containers': [container]})
    ...
    #template.setdefault('spec', {'containers': [container]})
    container.setdefault('args', ['--appendonly','yes', "--requirepass", REDIS_PASSWD])
    volumes = {}
    ...
    env.append({'name': 'REDIS_PASSWD', 'value': REDIS_PASSWD})
